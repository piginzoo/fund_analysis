...
def main(args):
    ...
    invest_data = filter_invest_by(data, args.period)
    # print(data.info())
    # print(data.describe())
    # print(invest_data)
...
def filter_invest_by(data, period):
    indices = []
    for date_index, one in data.iterrows():
        ...
        if period == conf.PERIOD_WEEK and date.weekday() == 1:
            # candidate_df.append(one)
            indices.append(date_index)

        # only invest at first day of each month
        if period == conf.PERIOD_MONTH and date.day == 1:
            # candidate_df.append(one)
            indices.append(date_index)
...
# python -m fund_analysis.auto_invest --code 519778 --start 2020-01-01 --end 2021-04-22 --period week
