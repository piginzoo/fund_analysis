...
def parse_successor_date(period, day, previous_line, next_line, amount):
    ...
    for i in range(delta.days + 1):
        ...
        if period == conf.PERIOD_MONTH and date.day == day:
            ...
            records.append(InvestRecord(date=date_str, amount=amount))
    ...
    for i, line in enumerate(lines):
        ...
        if date_str.startswith(conf.PERIOD_MONTH) or \
            ...
            if i < len(lines) - 1:
                ...
                records.append(parse_successor_date(period, day, previous_line, next_line, amount))
            else:  # if i am the last line
                yesterday = datetime.now() - timedelta(days=1)
                fake_yesterday_line = yesterday.strftime('%Y-%m-%d')+",0"
                records.append(parse_successor_date(period, day, previous_line, fake_yesterday_line, amount))
    return records
